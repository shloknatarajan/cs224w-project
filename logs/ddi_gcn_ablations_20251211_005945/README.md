# DDI GCN ablation summary

Runs executed via `run_gcn_ablation_fast.sh` (one run each, 200 epochs, eval every 10 epochs, device cuda:0). Metrics are final Hits reported by the script.
...
Best test performance came from E2 (2-layer, dropout 0.25, lr 0.005, batch 65,536).
