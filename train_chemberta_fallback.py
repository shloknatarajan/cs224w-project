"""
Train ChemBERTa-based models with learnable fallback embeddings for missing SMILES.

This script uses:
- ChemBERTa embeddings for nodes WITH SMILES
- Learnable fallback embedding for nodes WITHOUT SMILES
- This is better than using zeros for 46% of nodes with missing SMILES
"""
import torch
import logging
import os
...
log_dir = f'logs/chemberta_fallback_{timestamp}'
os.makedirs(log_dir, exist_ok=True)
log_filename = f'{log_dir}/chemberta_fallback.log'
...
# Load dataset with ChemBERTa embeddings + SMILES mask
...
logger.info(f"SMILES mask: {data.smiles_mask.sum().item():.0f} valid / {num_nodes} total ({data.smiles_mask.sum().item()/num_nodes*100:.1f}%)")
...
# Fallback model specific
USE_ID_EMBEDDINGS = True  # Helps disambiguate missing-SMILES nodes
ID_DIM = 64  # Dimension of ID embeddings (if used)
USE_STRUCTURAL_FALLBACK = True  # Use structural features instead of a single shared vector
...
# Train ChemBERTa-GCN-Fallback
logger.info("\n" + "=" * 80)
logger.info("Training ChemBERTa-GCN-Fallback")
logger.info("=" * 80)
gcn_model = ChemBERTaGCNFallback(
    num_nodes=num_nodes,
    ...
    id_dim=ID_DIM,
    use_structural_fallback=USE_STRUCTURAL_FALLBACK,
    num_structural_features=NUM_STRUCT_FEATURES
)
gcn_result = train_minimal_baseline(
    "ChemBERTa-GCN-Fallback",
    gcn_model,
    data,
    train_pos,
    valid_pos,
...
results['ChemBERTa-GCN-Fallback'] = gcn_result

# Train ChemBERTa-GraphSAGE-Fallback
logger.info("\n" + "=" * 80)
logger.info("Training ChemBERTa-GraphSAGE-Fallback")
logger.info("=" * 80)
sage_model = ChemBERTaGraphSAGEFallback(
    num_nodes=num_nodes,
    in_channels=IN_CHANNELS,
    hidden_dim=HIDDEN_DIM,
    num_layers=NUM_LAYERS,
...
results['ChemBERTa-GraphSAGE-Fallback'] = sage_result

# Train ChemBERTa-GraphTransformer-Fallback
logger.info("\n" + "=" * 80)
logger.info("Training ChemBERTa-GraphTransformer-Fallback")
logger.info("=" * 80)
transformer_model = ChemBERTaGraphTransformerFallback(
    num_nodes=num_nodes,
    in_channels=IN_CHANNELS,
    hidden_dim=HIDDEN_DIM,
    num_layers=NUM_LAYERS,
...
transformer_result = train_minimal_baseline(
    "ChemBERTa-GraphTransformer-Fallback",
    transformer_model,
    data,
    train_pos,
    valid_pos,
...
results['ChemBERTa-GraphTransformer-Fallback'] = transformer_result

# Train ChemBERTa-GAT-Fallback
logger.info("\n" + "=" * 80)
logger.info("Training ChemBERTa-GAT-Fallback")
logger.info("=" * 80)
gat_model = ChemBERTaGATFallback(
    num_nodes=num_nodes,
    in_channels=IN_CHANNELS,
    hidden_dim=HIDDEN_DIM,
    num_layers=NUM_LAYERS,
...
results['ChemBERTa-GAT-Fallback'] = gat_result

# Final results summary
logger.info("\n" + "=" * 80)
logger.info("FINAL RESULTS - CHEMBERTA FALLBACK MODELS")
logger.info("=" * 80)
logger.info("NOTE: These models use learnable fallback embeddings for 46% of nodes without SMILES")
logger.info("")
