"""
ChemBERTa-based Graph Transformer baseline
...
class ChemBERTaGraphTransformer(BaseModel):
    """
    Graph Transformer baseline with ChemBERTa embedding input features.

    Architecture:
    - Input: ChemBERTa embeddings (768-dim)
    - Projection: Linear layer to map 768 -> hidden_dim
    - GNN: 2-layer Graph Transformer with attention
    ...
    def __init__(self, in_channels=768, hidden_dim=128, num_layers=2,
        ...
        self.description = (
            f"ChemBERTa-Transformer ({num_layers} layers, {heads} heads, dropout={dropout}) | "
    ...
    def encode(self, edge_index, x=None):
        ...
        """
        if x is None:
            raise ValueError("ChemBERTaGraphTransformer requires input features x (ChemBERTa embeddings)")
