"""
ChemBERTa-based GraphSAGE baseline
...
class ChemBERTaGraphSAGE(BaseModel):
    """
    GraphSAGE baseline with ChemBERTa embedding input features.
    ...
    def __init__(self, in_channels=768, hidden_dim=128, num_layers=2,
        ...
        self.description = (
            f"ChemBERTa-GraphSAGE ({num_layers} layers, dropout={dropout}, {bn_str}) | "
    ...
    def encode(self, edge_index, x=None):
        ...
        """
        if x is None:
            raise ValueError("ChemBERTaGraphSAGE requires input features x (ChemBERTa embeddings)")
