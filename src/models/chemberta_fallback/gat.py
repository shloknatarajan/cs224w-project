"""
ChemBERTa-based GAT with learnable fallback embeddings for missing SMILES
"""
import torch
...
class ChemBERTaGATFallback(BaseModel):
    """
    GAT with ChemBERTa embeddings and learnable fallback for missing SMILES.
    """
    ...
    def __init__(self, num_nodes, in_channels=768, hidden_dim=128, num_layers=2,
        ...
        self.description = (
            f"ChemBERTa-GAT-Fallback ({num_layers} layers, {heads} heads, dropout={dropout}{id_str}{struct_str}) | "
        ...
        self.convs.append(GATConv(gnn_input_dim, hidden_dim // heads, heads=heads, concat=True))
    ...
    def encode(self, edge_index, x=None, smiles_mask=None, structural_features=None):
        ...
        """
        if x is None:
            raise ValueError("ChemBERTaGATFallback requires input features x")
