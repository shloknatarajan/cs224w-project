"""
Morgan fingerprint-based GAT baseline
...
class MorganGAT(BaseModel):
    """
    GAT baseline with Morgan fingerprint input features.
    ...
    def __init__(self, in_channels=2048, hidden_dim=128, num_layers=2,
        ...
        self.description = (
            f"Morgan-GAT ({num_layers} layers, {heads} heads, dropout={dropout}) | "
    ...
    def encode(self, edge_index, x=None):
        ...
        """
        if x is None:
            raise ValueError("MorganGAT requires input features x (Morgan fingerprints)")
