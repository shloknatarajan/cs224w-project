"""
Morgan fingerprint-based GCN baseline
...
class MorganGCN(BaseModel):
    """
    GCN baseline with Morgan fingerprint input features.
    ...
    def __init__(self, in_channels=2048, hidden_dim=128, num_layers=2,
        ...
        self.description = (
            f"Morgan-GCN ({num_layers} layers, dropout={dropout}) | "
    ...
    def encode(self, edge_index, x=None):
        ...
        """
        if x is None:
            raise ValueError("MorganGCN requires input features x (Morgan fingerprints)")
