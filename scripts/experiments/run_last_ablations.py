"""
Sweep launcher for final OGBL-DDI GCN ablations.

Runs the following experiments:
- E2 (dropout): 2 layers, 256 hidden, 0.5 dropout, 0.005 lr, 65536 batch size.
- E2 (dimensions): 2 layers, 512 hidden, 0.25 dropout, 0.005 lr, 65536 batch size.
- E2 (3 layer): 3 layers, 256 hidden, 0.25 dropout, 0.005 lr, 65536 batch size.

Each config runs for 200 epochs and logs to
logs/last_ablations_<timestamp>/<config_name>/config.log plus a sweep-level log.
"""
...
def setup_root_logger(log_dir: str) -> logging.Logger:
    ...
    return logging.getLogger("last_ablations")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run final ablations for OGBL-DDI GCNAdvanced models.")
    ...
    parser.add_argument("--eval_steps", type=int, default=10, help="Evaluation frequency.")
    ...
    log_dir = os.path.join("logs", f"last_ablations_{timestamp}")
